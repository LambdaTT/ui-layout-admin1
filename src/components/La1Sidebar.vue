  ...
  computed: {
    ...
    navigator() {
      ...
      if (this.NavItems && this.NavItems.length > 0) {
        navigator = JSON.parse(JSON.stringify(this.NavItems));
      } else if (this.DataServiceURI) {
        navigator = JSON.parse(
          JSON.stringify(this.$getService(this.DataServiceURI).getData()),
        );
      }
...
.sidebar-icon-html :deep(.fa-stack) {
  width: 2em;
  height: 2em;
  line-height: normal;
